-- ============================================================================
--  MODULO DFe - EMPRESA 20, FILIAL GO (ARQUIVO empresas/20_01)
-- ============================================================================
--  CNPJ 66.471.517/0020-30. Raiz 66471517, a mesma da matriz.
--
...
--
--  ==========================================================================
--   ATENCAO: DRENAGEM DE OUTRA ORDEM DE GRANDEZA
--  ==========================================================================
--  A empresa 20 recebeu 12.413 notas de entrada nos ultimos 90 dias, contra 892
--  da empresa 29 e 781 da 30 - medido em 14/09/2026 na Consinco de producao.
--
--  Na proporcao ja medida de 7,1 documentos por nota (nota + resumo + eventos):
--
--      ~88.000 documentos    ~1.760 consultas distNSU    ~44 ciclos
--      ~170 MB               ate 2 dias drenando
--
--  O acervo INTEIRO do modulo em 14/09/2026 era de 19.428 documentos. Esta
--  empresa sozinha o multiplica por cinco.
--
--  A drenagem em si e a parte segura: enquanto vem documento a SEFAZ nao devolve
--  656 - a empresa 29 fez 114 consultas seguidas a 30 segundos sem nenhum. O
--  risco comeca depois que o fluxo alcanca o fim da fila.
--
--  NAO ATIVAR junto com a empresa 17, que e ainda maior (~207.000 documentos).
--  Uma de cada vez, com o freio observado.
--
--  ==========================================================================
...
--
--  Ate 09/09/2026 nao concordavam: a empresa 20 nascia no 02_carga_inicial com
...
select e.cod_dfe_empresa,
       ...
       3,
       60,
       'fluxo criado pausado: Qive removida deste CNPJ em 14/09/2026; certificado compartilhado com a matriz',
       sysdate,
       'CADASTRO DFE'
...
--      linhas e nao da erro nenhum. A empresa 20 fica sem certificado, o motor
...
--      linhas de certificado para a 20. O rownum = 1 fecha isso.
declare
  qtd_origem number;
  qtd_ins    number;
begin
  ...
  if qtd_origem = 0 then
    ...
    dbms_output.put_line('       A empresa 20 fica SEM certificado; o dfe:monitorar vai');
    ...
    if qtd_ins = 0 then
      dbms_output.put_line('certificado da empresa 20 ja existia - nada a fazer.');
    else
      dbms_output.put_line('certificado copiado da empresa 1 para a 20.');
    end if;
  end if;
end;
...
select cod_certi_digital,
       ...
       created_by
  from poseidon.dpc_conta_certif_digital_emp
 where cod_empresa in (1, 20)
   and status = 1
 order by cod_empresa;

--  4d. Panorama: onde a 20 entra no conjunto. Ativos primeiro.
...
--
--      dfe:ingerir --empresa=20 --tipo=NFE --reposicionar-cursor=0 --confirmar
--
--  E confira antes, sem consumir cota nenhuma:
--
--      dfe:ingerir --dry-run --empresa=20
--
...
--
--      29_99_rollback_empresa_20_dbeaver.sql
-- ============================================================================
